<template>
  <f7-page>
    <f7-navbar title="Szukane" back-link="Back" sliding>
    	<f7-nav-right>          
    		<f7-button  v-if="isLogin" v-on:click="search">Szukam</f7-button> 
            <i class="f7-icons" v-on:click="openListMenu">favorites</i>
        </f7-nav-right>
    </f7-navbar>
    <f7-grid no-gutter>
    	<f7-col :width="gridSettings" v-for="pet in pets">
        	<pet-card :pet="pet" searched="true"></pet-card>
    	</f7-col>       
     </f7-grid>
...
export default {
	...
	methods: {
		...
		search: function(){			
			this.$router.load({url: '/search-form/'})			
		}
	}
}
</script>
